...
if ( ! function_exists( 'snipes_press_setup' ) ) :
	...
	function snipes_press_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'snipes_press' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'snipes_press_custom_background_args',
				array(
...
function snipes_press_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'snipes_press_content_width', 640 );
}
 ...
 */
function snipes_press_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'snipes_press' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'snipes_press' ),
...
add_action( 'widgets_init', 'snipes_press_widgets_init' );
...
function snipes_press_scripts() {
	...
	wp_enqueue_script( 'snipes_press-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
